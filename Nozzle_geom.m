...
%Input Values
data = readmatrix("CONSTANTS_KEVIN.xlsx");
p_1= 1.379*10^7; %Chamber Pressure
T_1= data(1,2) %Chamber Temperature
FT= data(3,2) %Thrust of Merlin 1D
m_dot= data(4,2) %Mass Flow Rate
ALT= data(5,2) %Desired Altitude of Rocket
g= data(6,2) %Coefficient of Heats
R= data(7,2) %Gas Constant for Desired Gas Refer to picture
...
b(1) = TR;
    ...
    xlswrite('Solidworks_points.xlsx',transpose(xw),'PTS','A1:A62');
    xlswrite('Solidworks_points.xlsx',transpose(yw),'PTS','B1:B62');
